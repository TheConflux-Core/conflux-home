...
Use `@elevenlabs/elevenlabs-js` for batch transcription. This is the officially supported approach.
...
The SDK accepts several `Uploadable` formats:

```javascript
// Buffer (recommended for in-memory audio)
file: { data: Buffer.from(audioBytes), filename: "audio.wav", contentType: "audio/wav" }
...
**IMPORTANT:** The older `{ name, data, mimeType }` format is NOT supported in v10+ SDK. Always use `{ data, filename, contentType }` or `{ path }`.
...
For live microphone streaming, use the WebSocket API directly:
...
|-------|----------|-------|
| `model_id` | Yes | Must be `scribe_v2_realtime` for realtime |
| `audio_format` | No | Default `pcm_16000` |
| `commit_strategy` | No | `manual` or `vad` |

**Audio message format:**
```json
{
  "message_type": "input_audio_chunk",
  "audio_base_64": "<base64 PCM 16kHz mono>",
  "commit": false,
  "sample_rate": 16000
}
```

**On stream stop (final flush):**
```json
{
  "message_type": "input_audio_chunk",
  "audio_base_64": "",
