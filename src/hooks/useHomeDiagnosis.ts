...
export function useHomeDiagnosis() {
  ...
  const diagnose = useCallback(async (symptom: string) => {
    ...
    setError(null);
    try {
      const result = await invoke<HomeDiagnosis>('home_diagnose_problem', { userId: user.id, symptom });
      setDiagnosis(result);
    } catch (e) {
      setError(String(e));
    } finally {
  ...
  const logProblem = useCallback(async (description: string) => {
    ...
    setError(null);
    try {
      const result = await invoke<HomeDiagnosis>('home_log_problem_natural', { userId: user.id, description });
      setDiagnosis(result);
    } catch (e) {
      setError(String(e));
    } finally {
