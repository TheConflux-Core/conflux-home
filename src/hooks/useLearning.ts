...
export function useLearningProgress(memberId: string | null) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const data = await invoke<LearningProgress>('learning_get_progress', { memberId: memberId });
      setProgress(data);
      setError(null);
    } catch (e) {
      setError(String(e));
...
export function useLearningActivities(memberId: string | null, limit: number = 50) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const data = await invoke<LearningActivity[]>('learning_get_activities', { memberId: memberId, limit });
      setActivities(data);
    } catch (e) {
...
export function useLearningGoals(memberId: string | null) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const data = await invoke<LearningGoal[]>('learning_get_goals', { memberId: memberId });
      setGoals(data);
    } catch (e) {
