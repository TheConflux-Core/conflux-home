...
export function useDreams() {
  ...
  const addDream = useCallback(async (id: string, title: string, description: string | null, category: string, targetDate: string | null, memberId?: string): Promise<Dream> => {
  ...
  const updateDream = useCallback(async (id: string, title: string, description: string | null, category: string, targetDate: string | null) => {
    await invoke('dream_update', { userId, id, title, description, category, targetDate });
    await load();
  }, [userId, load]);

  const getMilestones = useCallback(async (dreamId: string): Promise<DreamMilestone[]> => {
    return await invoke<DreamMilestone[]>('dream_get_milestones', { userId, dreamId });
  }, [userId]);
  ...
  const aiPlan = useCallback(async (dreamId: string, title: string, description: string | null, category: string, targetDate: string | null) => {
    await invoke<any>('dream_ai_plan', { userId, dreamId, title, description, category, targetDate });
    await load();
  }, [userId, load]);
