...
function FindingCard({ finding }: { finding: VulnFinding }) {
  ...
  const loadPreview = async () => {
    try {
      const preview = await invoke<any>('remediation_dry_run', { findingId: finding.id, source: 'viper' });
      setFixPreview(preview);
    } catch (err) { console.error('[Viper] dry run failed:', err); }
  };
  ...
  const handleFix = async () => {
    ...
    setFixing(true);
    try {
      const result = await invoke<any>('remediation_execute', { findingId: finding.id, source: 'viper' });
      setFixResult(result);
