...
/// Build the OpenAI-compatible request body for cloud_chat/cloud_chat_stream.
fn build_chat_request_body(
    ...
    if let Some(task) = task_type {
        ...
        body["task_type"] = serde_json::json!(effective_task);
    }
...
async fn call_minimax(request_body: serde_json::Value) -> Result<ModelResponse> {
    ...
    Ok(ModelResponse {
        content,
        model: parsed.model.unwrap_or_else(|| "minimax".to_string()),
...
/// Call the MiniMax API for streaming responses (fallback when Groq is rate-limited).
async fn call_minimax_stream(
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|e| anyhow::anyhow!("Failed to call MiniMax (stream): {}", e))?;

    let status = response.status();
    if !status.is_success() {
        let body = response.text().await.unwrap_or_default();
        anyhow::bail!("MiniMax stream error ({}): {}", status, body);
    }
    ...
    while let Some(chunk) = stream.next().await {
        let chunk = chunk.map_err(|e| anyhow::anyhow!("MiniMax stream read error: {}", e))?;
...
pub async fn cloud_chat(
    ...
    if !status.is_success() {
        ...
        // If rate-limited, try MiniMax as fallback before failing
        if status.as_u16() == 429 {
            log::warn!("[cloud_chat] Groq rate-limited, trying MiniMax fallback...");
            match call_minimax(request_body).await {
                Ok(resp) => return Ok(resp),
                Err(e) => {
                    log::error!("[cloud_chat] MiniMax fallback also failed: {}", e);
                    anyhow::bail!("Rate limit exceeded. MiniMax fallback failed: {}", e);
                }
            }
        }
...
pub async fn cloud_chat_stream(
    ...
    if !status.is_success() {
        ...
        // If rate-limited, try MiniMax as fallback before failing
        if status.as_u16() == 429 {
            log::warn!("[cloud_chat_stream] Groq rate-limited, trying MiniMax fallback...");
            match call_minimax_stream(request_body, messages, on_chunk).await {
                Ok(resp) => return Ok(resp),
                Err(e) => {
                    log::error!("[cloud_chat_stream] MiniMax fallback also failed: {}", e);
                    anyhow::bail!("Rate limit exceeded. MiniMax fallback failed: {}", e);
                }
            }
        }
...
/// Sync Studio API keys from Supabase studio_config table to local SQLite config.
/// Only writes keys that are NOT already set locally (preserves local overrides).
/// Called after Supabase auth succeeds.
pub async fn sync_studio_keys_from_cloud() -> Result<Vec<String>> {
    let resp = supabase_get("studio_config", "select=key,value")?
        .send()
        .await
        .map_err(|e| anyhow::anyhow!("Failed to fetch studio_config: {}", e))?;

    if !resp.status().is_success() {
        let status = resp.status();
        let body = resp.text().await.unwrap_or_default();
        // Non-fatal: table may not exist yet on older Supabase instances
        log::warn!("[SyncKeys] studio_config fetch returned {}: {}", status, body);
        return Ok(vec![]);
    }
    ...
    let rows: Vec<serde_json::Value> = resp
        .json()
        .await
        .map_err(|e| anyhow::anyhow!("Failed to parse studio_config: {}", e))?;
    ...
    for row in &rows {
        ...
        // Only write if not already set locally (preserves local overrides)
        let existing = db.get_config(key).ok().flatten().filter(|v| !v.is_empty());
        if existing.is_none() {
            if let Err(e) = db.set_config(key, value) {
                log::warn!("[SyncKeys] Failed to set {}: {}", key, e);
            } else {
                log::info!("[SyncKeys] Synced {} from cloud", key);
    ...
    if synced.is_empty() {
        log::info!("[SyncKeys] All keys already present locally");
    } else {
        log::info!("[SyncKeys] Synced {} keys from cloud", synced.len());
    }
